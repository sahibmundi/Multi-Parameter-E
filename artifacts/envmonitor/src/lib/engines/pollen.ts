...
function pollenActivityScore(input: PollenInput): number {
  ...
  // Temperature: pollen thrives 15–25°C
  ...
  // Humidity: dry conditions increase dispersal
  if (hum < 40) score += 20;
  else if (hum < 55) score += 10;
  else if (hum > 75) score -= 20;
  else if (hum > 90) score -= 35;

  // Rain: washes pollen out
  if (rain > 70) score -= 35;
  else if (rain > 40) score -= 20;
  else if (rain > 20) score -= 10;
  else if (rain < 10) score += 10; // dry → high pollen

  // Dust proxy for airborne particles
  if (dust > 50) score += 15;
  else if (dust > 25) score += 8;

  // High pressure → stable air → more pollen dispersal
...
export function calculatePollen(input: PollenInput): PollenResult {
  ...
  if (score <= 20) {
    recommendation = 'Safe for all outdoor activities. Very low pollen dispersal.';
  } else if (score <= 40) {
    recommendation = 'Generally safe outdoors. Low allergy risk today.';
  } else if (score <= 60) {
    recommendation = rain > 30
      ? 'Rainfall is reducing pollen. Moderate caution advised for allergy sufferers.'
      : 'Moderate pollen activity. Allergy sufferers should carry medication.';
  } else if (score <= 80) {
    recommendation = 'High pollen levels. Wear a mask outdoors. Keep windows closed.';
  } else {
    recommendation = hum < 40
      ? 'Very high pollen dispersal — dry, warm conditions are ideal for pollen spread. Avoid prolonged outdoor exposure.'
      : 'Very high pollen activity. Sensitive individuals should remain indoors.';
  }
