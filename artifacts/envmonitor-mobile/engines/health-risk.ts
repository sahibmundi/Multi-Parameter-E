...
export function calculateHealthRisks(input: HealthRiskInput): HealthRisk[] {
  ...
  // 1. Asthma
  {
    ...
    risks.push({ id: 'asthma', condition: 'Asthma', riskLevel: rl, color: riskColor(rl),
  ...
  // 2. Respiratory Irritation
  {
    ...
    const rl = riskLevel(s);
    risks.push({ id: 'resp_irritation', condition: 'Respiratory Irritation', riskLevel: rl, color: riskColor(rl),
      description: s > 50 ? 'Airway irritants detected at concerning levels.' : s > 25 ? 'Mild irritants present. Sensitive individuals may experience discomfort.' : 'Respiratory conditions appear normal.',
      triggers });
  }
  ...
  // 3. Allergic Rhinitis
  {
    ...
    const rl = riskLevel(s);
    risks.push({ id: 'allergic_rhinitis', condition: 'Allergic Rhinitis', riskLevel: rl, color: riskColor(rl),
  ...
  // 4. Bronchitis
  {
    ...
    if (co2 > 1000) triggers.push(`CO₂ ${co2.toFixed(0)} ppm`);
    const rl = riskLevel(s);
    risks.push({ id: 'bronchitis', condition: 'Bronchitis', riskLevel: rl, color: riskColor(rl),
  ...
  // 5. Air Pollution Exposure
  {
    ...
    const rl = riskLevel(s);
    risks.push({ id: 'air_pollution', condition: 'Air Pollution Exposure', riskLevel: rl, color: riskColor(rl),
      description: s > 75 ? 'Very unhealthy air. Limit all outdoor activities.' : s > 50 ? 'Unhealthy air. Sensitive groups should stay indoors.' : s > 25 ? 'Moderate pollution. Outdoor precautions advised.' : 'Air pollution within safe limits.',
      triggers: [`AQI ${aqi}`] });
  }
  ...
  // 6. Heat Stress
  {
    ...
    const s = Math.min(100, heatIndex > 54 ? 100 : heatIndex > 40 ? 75 : heatIndex > 32 ? 45 : heatIndex > 27 ? 20 : 0);
    const triggers: string[] = [];
    if (temp > 30) triggers.push(`Temp ${temp.toFixed(1)}°C`);
    if (hum > 60 && temp > 27) triggers.push(`Humidity ${hum.toFixed(0)}%`);
    const rl = riskLevel(s);
    risks.push({ id: 'heat_stress', condition: 'Heat Stress', riskLevel: rl, color: riskColor(rl),
      description: s > 75 ? 'Extreme heat stress. Risk of heat stroke.' : s > 50 ? 'High heat load. Hydrate frequently.' : s > 25 ? 'Moderate heat stress. Limit strenuous activity.' : 'Heat stress risk is low.',
      triggers });
  }
  ...
  // 7. Dehydration
  {
    ...
    const rl = riskLevel(s);
    risks.push({ id: 'dehydration', condition: 'Dehydration', riskLevel: rl, color: riskColor(rl),
  ...
  // 8. Eye Irritation
  {
    ...
    const rl = riskLevel(s);
    risks.push({ id: 'eye_irritation', condition: 'Eye Irritation', riskLevel: rl, color: riskColor(rl),
      description: s > 50 ? 'Chemical concentrations may cause significant eye irritation.' : s > 25 ? 'Mild eye irritation possible.' : 'Eye irritation risk is low.',
      triggers });
  }
  ...
  // 9. Skin Irritation
  {
    ...
    if (nh3 > 25) triggers.push(`NH₃ ${nh3.toFixed(1)} ppm`);
    const rl = riskLevel(s);
    risks.push({ id: 'skin_irritation', condition: 'Skin Irritation', riskLevel: rl, color: riskColor(rl),
      description: s > 50 ? 'VOC exposure may cause dermal irritation.' : s > 25 ? 'Mild skin irritation possible on prolonged exposure.' : 'Skin irritation risk is low.',
      triggers });
  }

  // 10. General Respiratory Discomfort
  {
    const s = Math.min(100,
      (co2 > 1000 ? 20 : 0) +
    ...
    const rl = riskLevel(s);
    risks.push({ id: 'resp_discomfort', condition: 'General Respiratory Discomfort', riskLevel: rl, color: riskColor(rl),
      description: s > 50 ? 'Multiple airway irritants present. Breathing may feel difficult.' : s > 25 ? 'Mild respiratory discomfort possible.' : 'Respiratory conditions are comfortable.',
      triggers });
  }
