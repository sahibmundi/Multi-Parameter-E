...
export function calculateAQI(input: AQIInput): AQIResult {
  // Prefer PMS7003 PM2.5 over optical dust sensor for AQI
  const pm25Source = input.pms25 ?? input.dust;
  const pm10Source = input.pms10;

  const pm25Aqi = pm25Source != null ? pm25ToAQI(pm25Source) : 0;
  const pm10Aqi = pm10Source != null
    ? Math.round(
        pm10Source > 50 ? linearScale(pm10Source, 50, 300, 100, 300)
          : pm10Source > 25 ? linearScale(pm10Source, 25, 50, 50, 100)
          : linearScale(pm10Source, 0, 25, 0, 50)
      )
    : 0;
  ...
  return { score, category, color, dominantPollutant: dominant.val > 0 ? dominant.name : 'None', severity };
}
