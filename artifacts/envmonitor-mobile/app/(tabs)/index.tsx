...
function processVoiceQuery(query: string, readings: Record<string, number>, lang: Lang): string {
  ...
  for (const cfg of SENSOR_CONFIGS) {
    ...
    if (keys.some((k) => q.includes(k))) {
      ...
      return lang === 'pa'
        ? `${sensorLabel('pa', cfg.id)} ${val.toFixed(1)} ${cfg.unit} ਹੈ। ਸਥਿਤੀ ${status} ਹੈ।`
        : lang === 'hi'
        ? `${sensorLabel('hi', cfg.id)} ${val.toFixed(1)} ${cfg.unit} है। स्थिति ${status} है।`
        : `${sensorLabel('en', cfg.id)} is ${val.toFixed(1)} ${cfg.unit}. Status is ${status}.`;
    }
  }
  ...
  return lang === 'pa'
    ? 'ਮੈਂ ਸਮਝ ਨਹੀਂ ਸਕਿਆ। ਤਾਪਮਾਨ, CO2, ਜਾਂ ਸਕੋਰ ਬਾਰੇ ਪੁੱਛੋ।'
    : lang === 'hi'
    ? 'मैं समझ नहीं पाया। तापमान, CO2, या स्कोर के बारे में पूछें।'
    : 'I did not understand. Ask about temperature, CO2, smoke, or score.';
}

function speak(text: string, lang: Lang) {
  const langCode = lang === 'pa' ? 'pa-IN' : lang === 'hi' ? 'hi-IN' : 'en-US';
  Speech.stop();
  Speech.speak(text, {
    language: langCode,
    pitch: 0.95,
...
export default function DashboardScreen() {
  ...
  const { readings, isLoading, isConnected, error, lastUpdated, refresh, lang, ch1Id, ch2Id, setVoiceResult, voiceQuery, voiceResponse } = useApp();
  ...
  const handleMic = useCallback(async () => {
    ...
    // On native devices, use expo-speech for response and simulate a query
    // For real STT, expo-speech only does TTS; we show a prompt-style input
    setIsListening(true);
    // Speak a prompt
    speak(
      lang === 'pa' ? 'ਕਿਰਪਾ ਕਰਕੇ ਆਪਣਾ ਸਵਾਲ ਕਹੋ।'
        : lang === 'hi' ? 'कृपया अपना प्रश्न बोलें।'
        : 'Please ask your question.',
      lang
    );
    // Auto-dismiss after 4s (in a real app, STT would close this)
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => { Haptics.selectionAsync(); refresh(); }} style={styles.refreshBtn}>
          {isLoading
            ...
            : <Feather name="refresh-cw" size={18} color="#00F5FF" />
          }
        </TouchableOpacity>
      </View>
            ...
            <View style={styles.insightCard}>
              ...
              {healthAlerts.slice(0, 3).map((a) => (
                <View key={a.id} style={[styles.alertRow, { borderLeftColor: LEVEL_COLORS[a.level] }]}>
                  <Text style={[styles.alertRowText, { color: LEVEL_COLORS[a.level] }]} numberOfLines={1}>
                    {a.title[lang]}
                  </Text>
                </View>
              ))}
              {healthAlerts.length === 0 && (
                <Feather name="check-circle" size={28} color="#00FF88" style={{ marginTop: 8 }} />
              )}
            </View>
          </View>
        )}
        ...
        <View style={styles.grid}>
          {SENSOR_CONFIGS.map((cfg) => (
            <View key={cfg.id} style={styles.gridCell}>
              <SensorCard
                id={cfg.id}
                value={readings?.[cfg.id as keyof typeof readings] as number | null ?? null}
                lang={lang}
                onPress={() => handleQuickQuery(cfg.id)}
              />
            </View>
          ))}
        </View>
...
const styles = StyleSheet.create({
  ...
  errorBanner: {
    ...
    backgroundColor: 'rgba(255,51,102,0.1)', borderWidth: 1,
    borderColor: 'rgba(255,51,102,0.3)', borderRadius: 10, padding: 12,
  },
  errorText: { flex: 1, color: '#FF3366', fontSize: 12 },
  scoreRow: { flexDirection: 'row', gap: 12 },
  scoreCard: {
    flex: 1, backgroundColor: '#0D1F3C', borderRadius: 14,
    borderWidth: 1, borderColor: 'rgba(0,245,255,0.15)',
    alignItems: 'center', justifyContent: 'center', padding: 14, gap: 6,
  },
  scoreLabel: { fontSize: 10, color: '#6B8CAE', fontWeight: '700', letterSpacing: 0.8 },
  insightCard: {
    flex: 2, backgroundColor: '#0D1F3C', borderRadius: 14,
    borderWidth: 1, borderColor: 'rgba(0,245,255,0.15)', padding: 14, gap: 6,
  },
  ...
  quickChip: {
    backgroundColor: 'rgba(0,245,255,0.1)', borderWidth: 1,
    borderColor: 'rgba(0,245,255,0.3)', borderRadius: 20,
    paddingHorizontal: 14, paddingVertical: 7,
  },
  quickChipText: { color: '#00F5FF', fontSize: 12, fontWeight: '600' },
  voiceCard: {
    backgroundColor: '#0D1F3C', borderRadius: 14,
    borderWidth: 1, borderColor: 'rgba(0,245,255,0.2)', padding: 14, gap: 8,
  },
  ...
  mic: {
    ...
    shadowColor: '#00F5FF', shadowOpacity: 0.5, shadowRadius: 12, elevation: 8,
  },
